...
| 13 | ~640-740 lines (~14%) dead/deprecated code across codebase | `cn_agent.ml`, `cn_io.ml`, `inbox_lib.ml`, `cn_ffi.ml`, `cn_lib.ml`, `cn_protocol.ml` | Maintenance burden, confusion for contributors |
...
| 24 | `run_commit` wildcard `_ ->` treats `git status --porcelain` exec failure as "has changes" | `cn_commands.ml:88` | Bug: git exec failure silently treated as dirty worktree |
| 25 | `send_thread` FSM transitions succeed even when underlying git operations fail silently — state/reality mismatch | `cn_mail.ml:274-368` | Silent state divergence if git push/commit fails |
| 26 | `run_daemon` has no graceful shutdown — no SIGTERM/SIGINT handler for `while true` loop | `cn_runtime.ml:458-522` | Process can only be killed, no cleanup |

### Low (nice to have)

| # | Finding | Location | Impact |
|---|---------|----------|--------|
| 27 | `cn_json.ml` does not handle `\uXXXX` escapes | `src/lib/cn_json.ml` | Edge case for unicode Telegram usernames |
| 28 | `Child_process.exec` loses error context | `src/ffi/cn_ffi.ml` | Debugging difficulty |
| 29 | Missing `string_of_sender_event`, `string_of_receiver_event` | `src/protocol/cn_protocol.ml` | Logging/debugging inconsistency |
| 30 | Archived docs lack "SUPERSEDED" banners | `docs/design/_archive/` | Discovery confusion |
| 31 | `spec/system/PROTOCOL.md` duplicates `docs/design/PROTOCOL.md` | `spec/system/` | Maintenance burden |
| 32 | `PLAN.md` is a stale snapshot | Root | Misleading |
| 33 | `cn_ffi.ml:Path.basename_ext` ignores its extension parameter | `src/ffi/cn_ffi.ml:96` | Misleading API — works by accident since all callers pass `.md` |
| 34 | `cn_hub.ml:slugify` truncates before lowercasing — can split multi-byte UTF-8 | `src/cmd/cn_hub.ml:39-58` | Invalid slugs for non-ASCII input |
| 35 | HUB.md vs SYSTEM.md directory structure mismatch (`threads/mail/inbox/` vs `threads/inbox/`) | `spec/system/HUB.md`, `spec/system/SYSTEM.md` | Spec inconsistency |
...
**Finding count: 3 critical, 7 high, 16 medium, 9 low (35 total).**
