...
|------|-------|-------|-----------|
| **alpha (PATTERN)** | A- | 0.85 | Strong structural consistency across modules. Clean layering (cli -> cmd -> lib/protocol/transport). Consistent naming conventions. Deductions: god module (cn_agent.ml, 731 lines), ~14% dead code (~640-740 lines), duplicated code across transport/lib layers, `log_action` no-op called everywhere, wrong weekly calculation, crontab replacement bug, FSM/git state mismatch in send_thread. |
| **beta (RELATION)** | B | 0.80 | Good pattern-relation alignment: FSMs match types, protocol spec matches code. Deductions: whitepaper says `peers.json` but code uses `peers.md`; ARCHITECTURE.md references non-existent `cn_daemon.ml`; SYSTEM.md vs AGENT.md contradict on agent interface; thread path three-way split (HEARTBEAT.md agents write to wrong dirs); STATELESS-AGENCY.md false implementation claims; GLOSSARY.md at v1.4.0 while code at v3.0.0; ROADMAP.md references JavaScript tooling; actor FSM disconnected from v3.0 runtime; CLI flags parsed but do nothing. |
...
| **C_Sigma** | **B+** | **0.83** | `(0.85 * 0.80 * 0.85)^(1/3) = 0.833`. A mature, well-structured project with good internal coherence. The main bottleneck is beta (spec-code drift across 6+ cross-reference inconsistencies, dead abstractions, false implementation claims in STATELESS-AGENCY.md, and feature gaps in parsed-but-inactive flags). |
...
8. **Archived docs not clearly labeled**: `docs/design/_archive/` contains 7 superseded documents but they don't have "SUPERSEDED" banners at the top — readers could find them via search and mistake them for current. *(Note: subsequent deep analysis found they DO have proper deprecation headers.)*

9. **STATELESS-AGENCY.md makes false implementation claims**: Claims `threads/wire/in/` is implemented ("All changes described in this document have been implemented as of 2026-02-20") but zero matches for `threads/wire/in` exist in source code. `cn_mail.ml:192` still uses `threads/in/`. Also claims `state: delegated` vocabulary is adopted but `cn_mail.ml:345` still writes `("state", "sent")`.

10. **THREAD-API.md is orphaned**: Not listed in `docs/README.md`, not covered by AUDIT.md. Claims status "Implementing" but commands described (`cn inbox show`, `cn thread list`, etc.) do not exist in CLI dispatch. Should be archived or status corrected.

11. **Thread path three-way split**: `threads/reflections/daily/` (ARCHITECTURE.md, CLI.md) vs `threads/daily/` (HEARTBEAT.md, AGENTS.md, WHITEPAPER.md) vs `threads/YYYYMMDD-daily.md` (GLOSSARY.md). An agent following HEARTBEAT.md would write to the wrong directories.

12. **GLOSSARY.md severely outdated**: Version header says "v1.4.0", missing all v3 terms (FSM, IO pair, packed context, trigger, op vocabulary, MCA, hub discovery, body consumption rules, projection layer). Contains deprecated entries (`memory/`, `state/practice/`).

13. **No v3.0 migration guide**: The only migration guide (`docs/how-to/MIGRATION.md`) covers v1.6.0. No guide exists for the OCaml rewrite, FSM addition, or OpenClaw → native runtime migration.

14. **ROADMAP.md critically outdated**: References v1.6.0, `npx @usurobor/cnos-setup` (JavaScript), marks `.gitattributes` and `cn.json` as not done when both exist.

15. **OpenClaw residual references**: `cn_agent.ml:289` has deprecated OpenClaw comment, `cn_agent.ml:468` has `/.openclaw/workspace/cnos` path in hub discovery fallback.
...
| 11 | STATELESS-AGENCY.md claims `threads/wire/in/` and `state: delegated` are implemented — neither is true | `docs/design/STATELESS-AGENCY.md` | False implementation claims in published design doc |
| 12 | Thread path three-way split: `threads/reflections/daily/` vs `threads/daily/` vs flat — agents following HEARTBEAT.md write to wrong directories | `src/agent/HEARTBEAT.md`, `docs/ARCHITECTURE.md`, `docs/design/WHITEPAPER.md` | Operational risk: agent writes to non-canonical paths |

### Medium (should plan to address)

| # | Finding | Location | Impact |
|---|---------|----------|--------|
| 13 | 6 modules with zero test coverage | `cn_system.ml`, `cn_gtd.ml`, `cn_mca.ml`, `cn_hub.ml`, `cn_ffi.ml`, `cn_fmt.ml` | Combined 1,069 LOC untested |
| 14 | No cram tests for `cn agent` modes | `test/cram/` | Critical runtime path untested end-to-end |
| 15 | ~640-740 lines (~14%) dead/deprecated code across codebase | `cn_agent.ml`, `cn_io.ml`, `inbox_lib.ml`, `cn_ffi.ml`, `cn_lib.ml`, `cn_protocol.ml` | Maintenance burden, confusion for contributors |
| 16 | `cn_io.ml` (193 lines) appears entirely superseded by `cn_mail.ml` | `src/transport/cn_io.ml` | Dead module still linked as dependency |
| 17 | Actor FSM not used by the new v3.0 runtime — only deprecated `run_inbound` uses it | `cn_runtime.ml`, `cn_protocol.ml` | Core protocol artifact disconnected from production code path |
| 18 | Parsed CLI flags (`--json`, `--quiet`, `--verbose`) are accepted but do nothing | `cn_lib.ml:419-425` | Silent feature gap — users expect these to work |
| 19 | Duplicated code: `split_status`/`is_retryable` between LLM and Telegram modules; `derive_name`/`starts_with`/`strip_prefix` between cn_lib and inbox_lib | Multiple files | DRY violation, divergence risk |
| 20 | ROADMAP.md critically outdated — references v1.6.0, JavaScript tooling, marks existing items as not done | `ROADMAP.md` | Project direction unclear, actively misleading to contributors |
| 21 | TSC grades appear self-awarded without formal evaluation | `CHANGELOG.md` | Quality metric credibility |
| 22 | GLOSSARY.md severely outdated — version says v1.4.0, missing all v3 terms, contains deprecated entries | `docs/reference/GLOSSARY.md` | Reference document unusable for v3.0 concepts |
| 23 | No v3.0 migration guide — only migration doc covers v1.6.0 | `docs/how-to/MIGRATION.md` | No documented path from OpenClaw to native runtime |
| 24 | THREAD-API.md orphaned — claims "Implementing" but commands don't exist, not indexed in docs/README.md | `docs/design/THREAD-API.md` | Misleading status on unimplemented feature |
| 25 | OpenClaw residual references in hub discovery fallback path | `cn_agent.ml:289,468` | Dead code references removed dependency |
| 26 | No opam caching in CI | `.github/workflows/ci.yml` | CI time unnecessarily long |
| 27 | No lint/format enforcement in CI | `.github/workflows/ci.yml` | Style consistency not enforced |
| 28 | Release binaries without checksums or signing | `.github/workflows/release.yml` | Supply-chain verification gap |
| 29 | Hand-constructed JSON in `run_out` meta.json risks injection if values contain quotes | `cn_agent.ml:407-416` | Should use `Cn_json.to_string` |
| 30 | `run_commit` wildcard `_ ->` treats `git status --porcelain` exec failure as "has changes" | `cn_commands.ml:88` | Bug: git exec failure silently treated as dirty worktree |
| 31 | `send_thread` FSM transitions succeed even when underlying git operations fail silently — state/reality mismatch | `cn_mail.ml:274-368` | Silent state divergence if git push/commit fails |
| 32 | `run_daemon` has no graceful shutdown — no SIGTERM/SIGINT handler for `while true` loop | `cn_runtime.ml:458-522` | Process can only be killed, no cleanup |

### Low (nice to have)

| # | Finding | Location | Impact |
|---|---------|----------|--------|
| 33 | `cn_json.ml` does not handle `\uXXXX` escapes | `src/lib/cn_json.ml` | Edge case for unicode Telegram usernames |
| 34 | `Child_process.exec` loses error context | `src/ffi/cn_ffi.ml` | Debugging difficulty |
| 35 | Missing `string_of_sender_event`, `string_of_receiver_event` | `src/protocol/cn_protocol.ml` | Logging/debugging inconsistency |
| 36 | `spec/system/PROTOCOL.md` duplicates `docs/design/PROTOCOL.md` | `spec/system/` | Maintenance burden |
| 37 | `PLAN.md` is a stale snapshot | Root | Misleading |
| 38 | `cn_ffi.ml:Path.basename_ext` ignores its extension parameter | `src/ffi/cn_ffi.ml:96` | Misleading API — works by accident since all callers pass `.md` |
| 39 | `cn_hub.ml:slugify` truncates before lowercasing — can split multi-byte UTF-8 | `src/cmd/cn_hub.ml:39-58` | Invalid slugs for non-ASCII input |
| 40 | HUB.md vs SYSTEM.md directory structure mismatch (`threads/mail/inbox/` vs `threads/inbox/`) | `spec/system/HUB.md`, `spec/system/SYSTEM.md` | Spec inconsistency |
| 41 | HANDSHAKE.md describes old manual git clone/push approach, not current `cn sync` automation | `docs/how-to/HANDSHAKE.md` | Outdated how-to guide |
| 42 | DOJO.md has naming format typo (`yyyyddmmhhmmss` — dd/mm swapped) and stale version (v1.2.3) | `docs/tutorials/DOJO.md` | Tutorial typo |
...
9. **Remove dead code** — `cn_io.ml` (193 lines, superseded), `inbox_lib.ml` action planning (~150 lines), deprecated functions in `cn_agent.ml` (~230 lines), unused `Http.get`/`Http.post` in `cn_ffi.ml`, `dir_of_thread_state` in `cn_protocol.ml`, unreachable `run_inbox_list`/`run_outbox_list` in `cn_gtd.ml`. Target: remove ~700 lines.
...
16. **Fix STATELESS-AGENCY.md implementation claims** — either implement `threads/wire/in/` and `state: delegated` in `cn_mail.ml`, or correct the "Implementation Status" to say these are proposed but not implemented.
17. **Standardize thread paths** — reconcile `threads/reflections/daily/` (ARCHITECTURE.md) vs `threads/daily/` (HEARTBEAT.md, AGENTS.md, WHITEPAPER.md). Update all docs to match the canonical path.
18. **Refresh GLOSSARY.md** — update version to v3.0.0, add v3 terms (FSM, IO pair, packed context, trigger, op vocabulary, MCA, hub discovery, body consumption rules, projection). Remove deprecated entries.
19. **Rewrite ROADMAP.md** — currently references v1.6.0 and JavaScript tooling. Update to reflect v3.0.0 and current priorities.
20. **Archive or correct THREAD-API.md** — orphaned doc claiming "Implementing" status for non-existent commands. Move to `_archive/` or update status.
21. **Create v3.0 migration guide** — document OpenClaw → native runtime transition, env var requirements, config format, and cron/daemon setup.
22. **Remove OpenClaw residual references** — `cn_agent.ml:289` deprecated comment and `cn_agent.ml:468` fallback path.
23. **Add opam caching to CI** — `actions/cache` for `~/.opam`.
24. **Add SHA256 checksums to release workflow**.
...
The project shows clear evidence of **evolutionary growth** through three generations — the transport layer (`cn_io.ml`, `inbox_lib.ml`) was the original implementation, later superseded by the command layer (`cn_mail.ml`, `cn_agent.ml`), and then again by the v3.0 runtime layer (`cn_runtime.ml`). The old layers are still linked but largely unused, contributing to the ~14% dead code figure. A cleanup pass removing the superseded transport code would meaningfully reduce complexity.
...
3. **Spec-code drift**: The doc-to-code ratio of 2.7:1 is remarkable but creates maintenance burden. Deep cross-reference analysis revealed 6+ major inconsistencies: thread path three-way split (ARCHITECTURE vs HEARTBEAT vs GLOSSARY), STATELESS-AGENCY.md false implementation claims, SYSTEM.md vs AGENT.md agent interface contradiction, HUB.md vs SYSTEM.md directory structure mismatch, WHITEPAPER.md `peers.json` vs code `peers.md`, and GLOSSARY.md frozen at v1.4.0. ROADMAP.md references JavaScript tooling that was removed 2+ major versions ago. These are not just cosmetic — an agent following HEARTBEAT.md will write reflections to non-canonical paths.

The self-awarded TSC grades in the CHANGELOG are aspirational — applying the actual TSC framework rigorously (which scores itself at C_Sigma = 0.238) would likely produce lower numbers. This audit's grade of B+ (0.83) is honest: strong architecture with real gaps in execution details and documentation currency.

For a project at version 3.0.0 with 86 commits and two active contributors, this is solid work with a clear path forward: fix the 3 critical issues, reconcile the 6+ cross-reference inconsistencies (especially the thread path three-way split and STATELESS-AGENCY.md false claims), remove the ~700 lines of dead code, and then systematically close the testing gaps.

**Finding count: 3 critical, 9 high, 20 medium, 10 low (42 total).**
