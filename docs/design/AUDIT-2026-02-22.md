...
|------|-------|-------|-----------|
| **alpha (PATTERN)** | A | 0.88 | Strong structural consistency across modules. Clean layering (cli -> cmd -> lib/protocol/transport). Consistent naming conventions and OCaml idioms. Marred by one god module (cn_agent.ml at 731 lines) and deprecated code still present. |
| **beta (RELATION)** | A- | 0.84 | Pattern-relation alignment is strong: docs match code, FSMs match implementation, protocol spec matches types. Deductions: whitepaper says `peers.json` but code uses `peers.md`; ARCHITECTURE.md references non-existent `cn_daemon.ml`; some doc-code drift in minor details. |
| **gamma (EXIT/PROCESS)** | A | 0.87 | Evolution is disciplined: 86 commits, clear version progression (v0.1 to v3.0), each release with coherent scope. CHANGELOG is detailed and version-stamped. CI enforces build+test. Deductions: no git tags, no release checksums, deprecated code retained without removal timeline. |
| **C_Sigma** | **A-** | **0.86** | `(0.88 * 0.84 * 0.87)^(1/3) = 0.863`. A mature, well-structured project with strong internal coherence. The main bottleneck is beta (doc-code drift in a few specifics). |
...
| # | Finding | Location | Impact |
|---|---------|----------|--------|
| 3 | ARCHITECTURE.md references non-existent `cn_daemon.ml` | `docs/ARCHITECTURE.md` | Doc-code mismatch confuses new contributors |
| 4 | WHITEPAPER.md claims `peers.json` but code uses `peers.md` | `docs/design/WHITEPAPER.md` | Spec-code mismatch |
| 5 | 731-line god module with mixed concerns | `cn_agent.ml` | Maintainability — auto-update logic interleaved with queue/execution |
| 6 | CI claims to test executable specs but does not run `@doc-test` | `.github/workflows/ci.yml` | Spec claim unverified |

### Medium (should plan to address)

| # | Finding | Location | Impact |
|---|---------|----------|--------|
| 7 | 6 modules with zero test coverage | `cn_system.ml`, `cn_gtd.ml`, `cn_mca.ml`, `cn_hub.ml`, `cn_ffi.ml`, `cn_fmt.ml` | Combined 1,069 LOC untested |
| 8 | No cram tests for `cn agent` modes | `test/cram/` | Critical runtime path untested end-to-end |
| 9 | Deprecated code without removal timeline | `cn_agent.ml:260-293, 610-731` | 115 lines of dead code increasing maintenance burden |
| 10 | ROADMAP.md outdated with completed items | `ROADMAP.md` | Project direction unclear to potential contributors |
| 11 | TSC grades appear self-awarded without formal evaluation | `CHANGELOG.md` | Quality metric credibility |
| 12 | No opam caching in CI | `.github/workflows/ci.yml` | CI time unnecessarily long |
| 13 | No lint/format enforcement in CI | `.github/workflows/ci.yml` | Style consistency not enforced |
| 14 | Release binaries without checksums or signing | `.github/workflows/release.yml` | Supply-chain verification gap |

### Low (nice to have)

| # | Finding | Location | Impact |
|---|---------|----------|--------|
| 15 | `cn_json.ml` does not handle `\uXXXX` escapes | `src/lib/cn_json.ml` | Edge case for unicode Telegram usernames |
| 16 | `Child_process.exec` loses error context | `src/ffi/cn_ffi.ml` | Debugging difficulty |
| 17 | Missing `string_of_sender_event`, `string_of_receiver_event` | `src/protocol/cn_protocol.ml` | Logging/debugging inconsistency |
| 18 | Archived docs lack "SUPERSEDED" banners | `docs/design/_archive/` | Discovery confusion |
| 19 | `spec/system/PROTOCOL.md` duplicates `docs/design/PROTOCOL.md` | `spec/system/` | Maintenance burden |
| 20 | `PLAN.md` is a stale snapshot | Root | Misleading |

---

## 8. Recommendations (Priority Order)

1. **Quote `install_dir` in shell commands** — one-line fix, eliminates injection surface.
2. **Create git tags** for all released versions (`v1.0.0` through `v3.0.0`).
3. **Fix ARCHITECTURE.md** — replace `cn_daemon.ml` reference with `cn_runtime.ml`.
4. **Update WHITEPAPER.md** — either migrate code to `peers.json` or update whitepaper to reflect `peers.md` reality.
5. **Extract `cn_update.ml`** from `cn_agent.ml` — reduces god module, improves testability.
6. **Add `@doc-test` target to CI** — or remove the claim from CNOS.md.
7. **Add tests for `cn_system.ml`, `cn_gtd.ml`** — these handle user-facing commands.
8. **Set removal version for deprecated code** — e.g., "Remove in v3.1.0".
9. **Add opam caching to CI** — `actions/cache` for `~/.opam`.
10. **Add SHA256 checksums to release workflow**.

---

## 9. Overall Assessment

cnos is a well-engineered OCaml project with disciplined architecture, strong type safety, and unusually thorough documentation for its size. The pure-core/effectful-shell separation is textbook, the FSM design is robust, and the test infrastructure (ppx_expect + cram) is appropriate.

The project's main weakness is **breadth over depth in testing** — the protocol layer is exhaustively tested, but several command modules and the runtime orchestrator lack unit coverage. The **doc-to-code ratio of 2.7:1** is remarkable but creates maintenance burden: a few spec-code mismatches have crept in.

The self-awarded TSC grades in the CHANGELOG are aspirational — applying the actual TSC framework rigorously (which scores itself at 0.238) would likely produce lower numbers. The intuition-level grades are reasonable as a directional signal but should not be conflated with formal C_Sigma measurement.

For a project at version 3.0.0 with 86 commits and two active contributors, this is high-quality work. The critical path to improvement is: fix the two critical issues (shell quoting, git tags), then systematically close the testing gaps in the 6 untested modules.
