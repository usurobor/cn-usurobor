...
- `src/cmd/cn_mail.ml:196-299` — Phase 0 hotfix in `materialize_branch`: pull clone main, explicit merge-base, fail closed on 0 or >1 candidates
- `src/cmd/cn_mail.ml:301-378` — `materialize_packet`: packet validation + dedup + exact materialization
- `src/cmd/cn_mail.ml:380-421` — `inbox_process`: dual-path (packets first, legacy branches second)
- `src/cmd/cn_mail.ml:458-490` — `send_thread`: packet creation via `Cn_packet.create_git_packet`
...
- [x] AC1: `materialize_branch` only materializes files introduced by the branch — explicit merge-base + `threads/in/` filter; packet path materializes exact validated payload only
...
- [x] AC3: Peer clone main is current before diff computation — `git pull origin main --ff-only` before diff
- [x] AC4: No silent content swap — fail-closed on ambiguity (Phase 0) + payload SHA-256 validation (Phase 1)
...
- beta: 4/4 — Send side creates packets → receive side validates them. Envelope fields match ref namespace bindings. Dedup index tracks all inbound packets. Legacy path gets Phase 0 hardening (pull main, explicit merge-base, fail closed). Packet lifecycle events cover fetched/validated/rejected/duplicate/equivocation/materialized.
- gamma: 4/4 — 16 tests cover envelope round-trip, validation rejection (5 failure modes), dedup (3 cases), ref parsing (3 cases), payload commitment determinism (2 cases), msg_id format. Dual-path inbox_process handles migration from branches to packets.
...
- Legacy branch send path removed in favor of packets; peers running old versions will not receive packets (migration doc needed)
- `cn_io.ml` still contains old `materialize_branch` / `sync_inbox` — dead code after this change lands
