...
let run_sync hub_path name =
  ...
  (* Heartbeat: update runtime.md and push *)
  update_runtime hub_path;
  let _ = Child_process.exec_in ~cwd:hub_path "git add state/runtime.md" in
  let commit_result = Child_process.exec_in ~cwd:hub_path "git commit -m 'heartbeat' --allow-empty" in
  (match commit_result with
   | Some _ -> 
       let _ = Child_process.exec_in ~cwd:hub_path "git push origin 2>/dev/null" in
       ()
   | None -> ());
